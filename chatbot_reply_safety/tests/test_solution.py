...
def test_every_jev_question_asks_about_the_experts_label_in_context(inputs, monkeypatch, capsys):
    run(monkeypatch, capsys, "jev-alone")
    script = [message([tool_use("ask_jev_reasons", {})], "tool_use"), message([text("ANSWER: 0.6")], "end_turn")]
    _, _, jev = run(monkeypatch, capsys, "claude-jev-decomposed", script)
    questions = [solution.OVERALL_QUESTION, *jev.seen[0][1].values()]
    assert len(questions) == 6
    for q in questions:
        assert "safety experts" in q and solution.CONTEXT_NOTE in q and "rater" not in q
